...
const FilmList = ({ selectedThemes, onFilmClick }: FilmListProps) => {
  const filteredFilms = selectedThemes.length > 0
    ? films.filter((film) =>
        film.keyIssues.some((issue) => selectedThemes.includes(issue))
      )
    : films;

  return (
    <div className="absolute bottom-4 left-4 right-4 z-10 bg-card/95 backdrop-blur-sm rounded-xl shadow-elevated p-4 max-h-48 overflow-y-auto">
